  ...
  public class DatingRepository : IDatingRepository
  {
    ...
    private async Task<IEnumerable<int>> GetUserLikes(int id, bool likers)
    {
      var user = await _context.Users
        .Include(x => x.Likees).Include(x => x.Likers)
        .FirstOrDefaultAsync(u => u.Id == id);

      return likers ? user.Likers.Where(u => u.LikeeId == id).Select(u => u.LikerId) :
    ...
    public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientId)
    {
      var messages = await _context.Messages
        ...
        .Where(m => m.RecipientId == userId && m.SenderId == recipientId && m.SenderDeleted == false ||
                    m.RecipientId == recipientId && m.RecipientDeleted == false && m.SenderId == userId)
